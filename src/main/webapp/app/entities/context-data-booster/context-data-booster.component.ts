...
@Component({
    selector: 'jhi-context-data-booster',
    templateUrl: './context-data-booster.component.html'
})
export class ContextDataBoosterComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.contextService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<ContextDataBooster[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/context-data-booster'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/context-data-booster', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ContextDataBooster) {
        return item.id;
    }
